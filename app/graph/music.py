...
`add_music(stitched_mp4, dest, niche, music_track, mode)` -> dest
...
stays light; install them yourself when you flip music_mode="generate".
"""
...
def _pick_track(niche: str | None, override: str | None) -> Path:
    ...
    for d in search_dirs:
        if not d.exists():
            continue
        tracks = sorted(p for p in d.iterdir() if p.is_file() and p.suffix.lower() in {".mp3", ".m4a", ".wav", ".ogg", ".flac"})
        if tracks:
...
async def _generate_track(niche: str | None, duration: float, dest: Path) -> Path:
    ...
    # MusicGen-small generates ~32k samples per second; tokens ~= 50/sec.
    ...
    # Trim/fade and convert to AAC so the mux is uniform.
    fade = 1.5
    args = [
        "ffmpeg", "-y", "-loglevel", "error",
        "-i", str(raw_wav),
