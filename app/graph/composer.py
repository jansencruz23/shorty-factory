...
Pattern: a plain ChatOpenAI is wrapped once with `with_structured_output(Storyboard)`
into `structured_llm`. Invoking it returns a parsed `Storyboard` Pydantic
instance directly — no manual parsing.
...
calling, swap to `method="json_mode"` (and the format instructions in the
system prompt will still steer it).
"""
